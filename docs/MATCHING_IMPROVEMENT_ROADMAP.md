...
لم أجد ملفاً باسم `prompt_for_ai.md` داخل المشروع. قبل تنفيذ أي تعديل على
مطالبات الذكاء الاصطناعي يجب إما إضافة الملف أو تحديد مكانه الصحيح، ثم تثبيت
قواعده كجزء من الاختبارات.
...
```bash
python run_tests.py
python run_matcher.py --no-ai --limit 200
```
