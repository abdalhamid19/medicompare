...
## حدود الكود الإلزامية

### عدد الأسطر في الملف

| النوع | الحد الأقصى |
|---|---|
| ملف Python عادي | 200 سطر |
| ملف pipeline/منسق | 300 سطر |
| ملف اختبار | 150 سطر |

عند تجاوز الحد: قسّم الملف إلى وحدات أصغر بمسؤولية واحدة.

### طول السطر

- **الحد الأقصى**: 88 حرف (متوافق مع Black)
- لا استثناءات إلا للروابط الطويلة أو مسارات ملفات لا يمكن كسرها.
...
|---|---|
| دالة عادية | 30 سطر |
| دالة hot path | 20 سطر |
| دالة اختبار | 40 سطر |

قواعد:

- لا دالة تتجاوز 50 سطر تحت أي ظرف.
- إذا احتجت أكثر: استخرج helper functions بأسماء واضحة.
- الدالة الطويلة = مسؤوليات متعددة = صعوبة الاختبار والصيانة.
- `run_matching()` و `run_ai_verification()` و `run_ai_search_unmatched()` حالياً تتجاوز الحد ويجب تقسيمها.

### قواعد إضافية

- **معاملات الدالة**: لا أكثر من 5 معاملات. استخدم config object أو dataclass عند الحاجة.
- **عمق التداخل (nesting)**: لا أكثر من 3 مستويات. استخرج دالة عند الحاجة.
- **الـ imports**: فقط في أعلى الملف. لا import داخل دالة إلا عند lazy loading ضروري.
