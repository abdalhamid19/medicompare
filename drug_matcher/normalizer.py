...
_DOSAGE_RE = re.compile(r"(\d+(?:\.\d+)?(?:\s\d{3})?)\s*(MG|MCG|I\s*U|IU|%)(?=$|\s)", re.IGNORECASE)
...
def parse_drug(name: str) -> DrugComponents:
    ...
    dosage_nums = tuple(m[0] for m in d_matches)
    ...
    for w in words:
        if re.search(r"\d", w):
            break
        if w in FORM_PREFIXES or w in FORM_WORDS or w in NOISE_WORDS:
            break
...
def components_match(d: DrugComponents, m: DrugComponents, brand_prefix_min: int = 4) -> tuple[bool, str]:
    ...
    if d_clean and m_clean:
        prefix_len = min(len(d_clean), len(m_clean), max(brand_prefix_min, int(min(len(d_clean), len(m_clean)) * 0.75)))
        prefix_len = min(prefix_len, len(d_clean), len(m_clean))
        if prefix_len > 0 and d_clean[:prefix_len] != m_clean[:prefix_len]:
            if d_clean not in m_clean and m_clean not in d_clean:
